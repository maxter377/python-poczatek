...
class Order:
    ...
    def __init__(self, client_first_name, client_last_name, order_elements=None):
        ...
        if order_elements is None:
            order_elements = []
        if len(order_elements) >Order.MAXIMUM_ORDER_ELEMENTS:
            order_elements = order_elements[:Order.MAXIMUM_ORDER_ELEMENTS]
    ...
    def add_product_to_order_price(self, product, quantity):
        if len(self._order_elements) < Order.MAXIMUM_ORDER_ELEMENTS:
            new_element = OrderElement(product, quantity)
            self._order_elements.append(new_element)
            self.total_price = self._calculate_total_price()
        else:
            print(f"Przekroczono maksymalną dopuszczalną liczbę produktów: {Order.MAXIMUM_ORDER_ELEMENTS}")
...
def generate_order(number_of_product):
    ...
    order = Order(client_first_name="Fernando", client_last_name="Alonso", order_elements=order_elements)
    return order